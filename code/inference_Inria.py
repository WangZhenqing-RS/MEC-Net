...
if __name__ == "__main__":
    ...
    for decoder_name in decoder_name_list:
        ...
        model_paths = [f"../model/{model.name}_Inria_Building_Dataset.pth"]
        output_dir = f'../data/Inria_Building_Dataset/test/pred_{model.name}'
        if not os.path.exists(output_dir): os.makedirs(output_dir)
        
        image_paths = glob.glob(r'../data/Inria_Building_Dataset/test/image/*.tif')
        batch_size = 4
        num_workers = 8
    ...
    # model_paths = [f"../model/{model.name}_Inria_Building_Dataset_build_building.pth"]
    # output_dir = f'../data/Inria_Building_Dataset/val/pred_{model.name}_build_building'
    # if not os.path.exists(output_dir): os.makedirs(output_dir)
    
    # image_paths = glob.glob(r'../data/Inria_Building_Dataset/val/image/*.tif')
    # batch_size = 16
    # num_workers = 8
    # test_loader = get_dataloader(image_paths, None, "test", is_MEC,
    #                               batch_size, False, num_workers)
    ...
    # model_paths = [f"../model/{models[0].name}_Inria_Building_Dataset.pth",
    #                f"../model/{models[1].name}_Inria_Building_Dataset.pth",
    #                f"../model/{models[2].name}_Inria_Building_Dataset.pth",
    #                f"../model/{models[3].name}_Inria_Building_Dataset.pth"]
    
    # output_dir = '../data/Inria_Building_Dataset/val/pred_fusion'
    # if not os.path.exists(output_dir): os.makedirs(output_dir)
