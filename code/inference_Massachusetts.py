...
if __name__ == "__main__":
    ...
    for decoder_name in decoder_name_list:
        ...
        model_paths = [f"../model/{model.name}_Massachusetts_Building_Dataset.pth"]
        output_dir = f'../data/Massachusetts_Building_Dataset/test/pred_{model.name}'
        if not os.path.exists(output_dir): os.makedirs(output_dir)
        
        image_paths = glob.glob(r'../data/Massachusetts_Building_Dataset/test/image/*.tif')
        batch_size = 16
        num_workers = 8
    ...
    model_paths = [f"../model/{model.name}_Massachusetts_Building_Dataset_build_building.pth"]
    output_dir = f'../data/Massachusetts_Building_Dataset/test/pred_{model.name}_build_building'
    if not os.path.exists(output_dir): os.makedirs(output_dir)
    ...
    model_paths = [f"../model/{models[0].name}_Massachusetts_Building_Dataset.pth",
                   f"../model/{models[1].name}_Massachusetts_Building_Dataset.pth",
                   f"../model/{models[2].name}_Massachusetts_Building_Dataset.pth",
                   f"../model/{models[3].name}_Massachusetts_Building_Dataset.pth"]
    
    output_dir = '../data/Massachusetts_Building_Dataset/test/pred_fusion'
    if not os.path.exists(output_dir): os.makedirs(output_dir)
